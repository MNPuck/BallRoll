...
public class WorldController extends InputAdapter {
	...
	private void initLevel() {
		...
		cameraHelper.setPosition(Constants.GAMEBOARD_WIDTH * .5f, Constants.GAMEBOARD_HEIGHT * .5f - .533f);
		cameraPosition = cameraHelper.getPosition();
		
		level = new Level(levelNumber, cameraPosition);
		...
		if (!level.returnIsGameOver() &&
			...
			if (!gameOver) {
				...
				if (firstTime) {
					...
					firstTime = false;
					cameraStartPosition.x = cameraPosition.x;
					cameraStartPosition.y = cameraPosition.y;
					
				}
			
				level.update(deltaTime, cameraPosition, cameraStartPosition, isLeftClicked, scrCoords);
